...
! -----------------------------------------------------------------------
!>\defgroup cires_ugwp_v0 CIRES Unified Gravity Wave Physics (UGWP) Version 0 
...
!! is represented by four GW-solvers, which is introduced in Lindzen 
!! (1981) \cite lindzen_1981, Hines (1997) \cite hines_1997, Alexander 
!! and Dunkerton (1999) \cite alexander_and_dunkerton_1999, and Scinocca 
!! (2003) \cite scinocca_2003. The major modification of these GW 
!! solvers is represented by the addition of the background dissipation 
!! of temperature and winds to the saturation criteria for wave 
!! breaking. This feature is important in the mesosphere and thermosphere
!! for WAM applications and it considers appropriate scale-dependent 
!! dissipation of waves near the model top lid providing the momentum 
!! and energy conservation in the vertical column physics (Shaw and 
!! Shepherd 2009 \cite shaw_and_shepherd_2009). In the UGWP-v0, the 
!! modification of Scinocca (2003) \cite scinocca_2003 scheme for NGWs
!! with non-hydrostatic and rotational effects for GW propagations and 
!! background dissipation is represented by the subroutine \ref fv3_ugwp_solv2_v0.
!! In the next release of UGWP, additional GW-solvers will be implemented
!! along with physics-based triggering of waves and stochastic approaches
!! for selection of GW modes characterized by horizontal phase velocities,
!! azimuthal directions and magnitude of the vertical momentum flux (VMF).
!!
...
!! The Fortran subroutine \ref slat_geos5_tamp describes the latitudinal
!! shape of VMF-function as displayed in Figure 3 of Molod et al. 
!! (2015) \cite molod_et_al_2015. It shows that the enhanced values 
!! of VMF in the equatorial region gives opportunity to simulate the
...
!! of current satellite missions. After delivery of the UGWP-code, 
!! the EMC group developed and tested approach to modulate the zonal 
!! mean NGW forcing by 3D-distributions of the total precipitation as
!! a proxy for the excitation of NGWs by convection and 
!! the vertically-integrated  (surface - tropopause) Turbulent Kinetic
!! Energy (TKE). The verification scores with updated NGW forcing, as 
!! reported elsewhere by EMC researchers, display noticeable improvements 
!! in the forecast scores produced by FV3GFS configuration extended into the mesosphere.
!!
